...
describe("RECOMMENDATIONS SERVICE TESTS - GET RANDOM FUNCTION", () => {
  it(" Should create object with 'gt' or 'lte' filter and return video", async () => {
    ...
    jest
      ...
      .mockImplementationOnce((obj): any => {
        if (
          obj.score === 10 &&
          (obj.scoreFilter === "gt" || obj.scoreFilter === "lte")
        ) {
          return [video];
        }
  ...
  it("Should throw error when findAll with no constraints and findAll with filter are null", async () => {
    const obj = Factory.Recomends.video();
    const video = {
      id: 5,
      name: obj.name,
...
describe("RECOMMENDATIONS SERVICE TESTS - GET FUNCTION", () => {
  it("Should call repositorie with amount parameter", async () => {
    const obj = Factory.Recomends.video();
    const video = {
      id: 4,
      name: obj.name,
...
describe("RECOMMENDATIONS SERVICE TESTS - GET TOP FUNCTION", ()=> {
  it("Should call 'getAmountByScore' repository with amount number", async ()=> {
      const amount = 10;
      const obj = Factory.Recomends.video();
      const video = {
        id: 4,
      ...
      .mockImplementationOnce((amount):any => {
        if(amount===10) {
          return [video]
        }
        throw {type:"Invalid amount call"}
      });
